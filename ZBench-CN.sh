# -*- shell-script -*-
# ZBench-CN.sh — 随本仓库收录（vendored），由 vps-benchkit 菜单「选项 2」调用。
# 上游仓库: https://github.com/FunctionClub/ZBench
# 上游文件: https://github.com/FunctionClub/ZBench/blob/master/ZBench-CN.sh
# Raw: https://raw.githubusercontent.com/FunctionClub/ZBench/master/ZBench-CN.sh
#
# 安全相关环境变量（可选）:
...
_zb_wget() {
    if [[ "${ZBENCH_WGET_INSECURE:-0}" == "1" ]]; then
...
# Install Besttrace（优先使用仓库 vendor/zbench）
if [[ -f "${ZB_VENDOR}/besttrace" ]]; then
    cp -f "${ZB_VENDOR}/besttrace" /tmp/besttrace
elif [ ! -e '/tmp/besttrace' ]; then
    echo "Installing Besttrace......"
    dir=$(pwd)
    cd /tmp/ || exit 1
    _zb_wget -N https://raw.githubusercontent.com/FunctionClub/ZBench/master/besttrace > /dev/null 2>&1
    cd "$dir" || true
fi
...
# Install Speedtest
if [[ -f "${ZB_VENDOR}/speedtest.py" ]]; then
    cp -f "${ZB_VENDOR}/speedtest.py" /tmp/speedtest.py
elif [ ! -e '/tmp/speedtest.py' ]; then
    echo "Installing SpeedTest......"
    dir=$(pwd)
    cd /tmp/ || exit 1
    _zb_wget -N https://raw.githubusercontent.com/sivel/speedtest-cli/master/speedtest.py > /dev/null 2>&1
    cd "$dir" || true
fi
...
# Install Zping-CN
if [[ -f "${ZB_VENDOR}/ZPing-CN.py" ]]; then
    cp -f "${ZB_VENDOR}/ZPing-CN.py" /tmp/ZPing-CN.py
elif [ ! -e '/tmp/ZPing-CN.py' ]; then
    echo "Installing ZPing-CN.py......"
    dir=$(pwd)
    cd /tmp/ || exit 1
    _zb_wget -N https://raw.githubusercontent.com/FunctionClub/ZBench/master/ZPing-CN.py > /dev/null 2>&1
    cd "$dir" || true
fi
...
if [[ -f "${ZB_VENDOR}/Generate.py" ]]; then
    cp -f "${ZB_VENDOR}/Generate.py" /tmp/Generate_zbench.py
else
    _zb_wget -N -O /tmp/Generate_zbench.py https://raw.githubusercontent.com/FunctionClub/ZBench/master/Generate.py >> /dev/null 2>&1
fi
python /tmp/Generate_zbench.py && rm -f /tmp/Generate_zbench.py && cp /root/report.html /tmp/report/index.html
...
if [[ "${ZBENCH_UPLOAD:-0}" == "1" ]]; then
    ...
    echo "未向 zbench 云端上传测评数据（默认）。若需在线报告链接，请设置 ZBENCH_UPLOAD=1 后重新运行。"
fi
echo "您的测评报告已保存在 /root/report.html"

# 临时 HTTP 预览（默认关闭；启用时仅监听 127.0.0.1，建议 SSH 端口转发访问）
if [[ "${ZBENCH_HTTP_REPORT:-0}" == "1" ]]; then
  while :; do echo
    read -p "你想现在通过本机 HTTP 查看测评报告吗? [y/n]: " ifreport
  ...
  if [[ $ifreport == 'y' ]];then
      ...
      if command -v python3 >/dev/null 2>&1; then
        echo "HTTP 仅监听 127.0.0.1:8001。本地浏览器请先建立转发，例如: ssh -L 8001:127.0.0.1:8001 root@<VPS公网IP>"
        echo "然后打开 http://127.0.0.1:8001/index.html ，按 Ctrl+C 结束服务。"
        cd /tmp/report || exit 1
        python3 -m http.server 8001 --bind 127.0.0.1
      else
        echo "未找到 python3，无法启动安全绑定的 HTTP 服务。请直接查看 /root/report.html 或安装 python3。"
      fi
  fi
else
  echo "未启用临时 HTTP 预览（默认）。若需要，请设置 ZBENCH_HTTP_REPORT=1 后重新运行。"
fi
